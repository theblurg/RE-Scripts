...
# leveraging the Carberp source code. 
# This code was modified from the origianl Unit 42 Project: https://github.com/pan-unit42/public_tools/blob/master/teslacrypt/deobfuscate_api_calls.py
# Modified by: theblurg
...
for api_name,api_value in apis.items():
	#print "%s" % hex(api_value).rstrip("L").lstrip("0x")
	address = find_pattern("%s" % hex(api_value).rstrip("L").lstrip("0x"))
	for x in address:
		print "%s : %x" % (api_name, x)
		func = idaapi.get_func(x)
		# We must consider the case when the compiler decides to inline
		# the caberp function hashing algorithm. If the function is
		# very large then it's likely that the caberp stuff has been
		# inlined.
		if ((func.endEA - func.startEA) < 60) :
			idc.MakeName(func.startEA, api_name)
		else:
			print "Function: %s was inlined. Function hash found at: %x" % (api_name,api_value)
